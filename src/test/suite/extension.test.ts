...
suite("Extension Test Suite", function() {
   ...
   * Uses XDG_CONFIG_HOME which is checked by getUserConfigDirectory() via process.env.
  ...
  function setupTempConfigDir(): { tempConfigDir: string; dprintConfigDir: string; cleanup: () => void } {
    const tempConfigDir = path.join(process.env.TMPDIR || "/tmp", `dprint-test-config-${Date.now()}`);
    const dprintConfigDir = path.join(tempConfigDir, "dprint");
    fs.mkdirSync(dprintConfigDir, { recursive: true });

    const originalXdgConfigHome = process.env.XDG_CONFIG_HOME;

    // Use XDG_CONFIG_HOME which getUserConfigDirectory() checks via process.env
  ...
  function removeWorkspaceConfig() {
    // workspaceRoot is guaranteed to be defined by the guard check at suite initialization
  ...
  suite("User-Level Config", function() {
    ...
    test("checkUserLevelConfig setting disables user-level lookup", async () => {
      ...
      try {
        ...
        // Try to format - should NOT change since no config found
        await context.formatCommand(doc.uri);
